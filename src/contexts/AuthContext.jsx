...
// Debug Appwrite configuration (only in development)
...
export const AuthProvider = ({ children }) => {
  ...
  const checkUser = async () => {
    try {
      if (import.meta.env.DEV) console.log('🔍 Checking existing session...');
      const session = await account.get();
      if (import.meta.env.DEV) console.log('✅ Existing session found:', session.email);
      setUser(session);
      
      // Check if user is the super admin
      const adminStatus = session.email === SUPER_ADMIN_EMAIL;
      setIsAdmin(adminStatus);
      if (import.meta.env.DEV) console.log('🔍 Admin status from existing session:', adminStatus);
    } catch (error) {
      if (import.meta.env.DEV) console.log('ℹ️ No existing session found');
      setUser(null);
      setIsAdmin(false);
    } finally {
      setLoading(false);
  ...
  const checkAdminStatus = async (userData = null) => {
    try {
      ...
      if (import.meta.env.DEV) console.log('🔍 Checking admin status for user:', currentUser?.email);
      
      // Only allow specific super admin email
      if (currentUser && currentUser.email === SUPER_ADMIN_EMAIL) {
        if (import.meta.env.DEV) console.log('✅ User is super admin');
        return true;
      }
      
      if (import.meta.env.DEV) console.log('❌ User is not super admin');
      return false;
    } catch (error) {
  ...
  const login = async (email, password) => {
    try {
      ...
      if (email !== SUPER_ADMIN_EMAIL) {
        if (import.meta.env.DEV) console.log('❌ Email not authorized for admin access');
        return { success: false, error: 'Access denied. Only super admin can access admin panel.', notAdmin: true };
      }
      
      if (import.meta.env.DEV) console.log('✅ Email authorized, attempting Appwrite login...');
      
      // Appwrite SDK v13 method name
      await account.createEmailSession(email, password);
      if (import.meta.env.DEV) console.log('✅ Appwrite session created successfully');
      
      const userData = await account.get();
      if (import.meta.env.DEV) console.log('✅ User data retrieved:', userData.email);
      setUser(userData);
      
      // Check admin status after login (pass userData directly)
      const adminStatus = await checkAdminStatus(userData);
      if (import.meta.env.DEV) console.log('✅ Admin status check:', adminStatus);
      setIsAdmin(adminStatus);
      
      // If trying to access admin but not an admin, return error
      if (!adminStatus) {
        if (import.meta.env.DEV) console.log('❌ Admin status check failed');
